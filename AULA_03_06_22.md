...
A tag DOCTYPE não precisa de um fechamento como outras tags, como no caso de ```html <p>``` e ```html <strong>```. A versão mais recente do HTML (versão 5) deve ser escrita apenas como html, não
...
```html <title>Barbearia Alura</title>```
